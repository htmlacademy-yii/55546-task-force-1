...
<section class="menu-toggle">
    <ul class="menu-toggle__list">
        <li class="menu-toggle__item menu-toggle__item--completed <?= $status === Task::STATUS_COMPLETED ? $currentItemClass : '' ?>">
            ...
            <?= Html::a('Завершённые',"my-list?status=". Task::STATUS_COMPLETED); ?>
        </li>
        <li class="menu-toggle__item menu-toggle__item--new <?= $status === Task::STATUS_NEW ? $currentItemClass : '' ?>">
            <div class="menu-toggle__svg-wrapper">
        ...
        <li class="menu-toggle__item menu-toggle__item--active <?= $status === Task::STATUS_EXECUTION ? $currentItemClass : '' ?>">
            <div class="menu-toggle__svg-wrapper">
            ...
            <?= Html::a('Активные',"my-list?status=". Task::STATUS_EXECUTION); ?>
        </li>
        <li class="menu-toggle__item menu-toggle__item--canceled <?= $status === Task::STATUS_CANCELED ? $currentItemClass : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <?= Html::a('Отменённые',"my-list?status=". Task::STATUS_CANCELED); ?>
        </li>
        <li class="menu-toggle__item menu-toggle__item--hidden <?= $status === Task::STATUS_EXPIRED ? $currentItemClass : '' ?>">
            <div class="menu-toggle__svg-wrapper">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <?= Html::a('Просроченные',"my-list?status=". Task::STATUS_EXPIRED); ?>
        </li>
    </ul>
</section>
        ...
        <?php foreach ($tasks as $task): ?>
            <div class="new-task__card">
                <div class="new-task__title">
                    <?= Html::a("<h2>" . Html::encode($task->title) . "</h2>", $task->getCurrentTaskUrl(), ['class' => 'link-regular']); ?>
                    <?php if($task->category): ?>
                        <?= Html::a("<p>" . Html::encode($task->category->title) . "</p>", Task::getUrlTasksByCategory($task->category->id), ['class' => 'new-task__type link-regular']); ?>
                    <?php endif; ?>
                </div>
                ...
                <?php if(($task->status === Task::STATUS_COMPLETED || $task->status === Task::STATUS_FAILING) && $task->executor): ?>
                    <div class="feedback-card__top ">
                        <?= Html::a("<img src='{$task->executor->userData->getAvatar()}' width='36' height='36'>", $task->executor->getCurrentUserUrl()); ?>
                        <div class="feedback-card__top--name my-list__bottom">
                            <p class="link-name">
                                <?= Html::a(Html::encode($task->executor->login), $task->executor->getCurrentUserUrl(), ['class' => 'link-regular']); ?>
                            </p>
