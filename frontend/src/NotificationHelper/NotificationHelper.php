...
class NotificationHelper
{
     ...
     * */
    public static function taskRespond(ActiveRecord $authorTask, ActiveRecord $task): void
    {
        self::send($authorTask, $task, [
            'type' => EventRibbon::TYPE_NEW_TASK_RESPOND,
            'messageEvent' => 'Новый отклик к заданию',
            'titleMail' => "Новый отклик к вашему заданию " . Html::encode($task->title),
            'messageMail' => "<p>Был оставлен новый отклик к вашему заданию " . Html::a(Html::encode($task->title), "http://yii-taskforce/tasks/view/$task->id") .".</p>",
        ]);
    }
     ...
     * @param ActiveRecord $task - объект задачи
     * */
    public static function taskStart(ActiveRecord $executorTask, ActiveRecord $task): void
    {
        self::send($executorTask, $task, [
            'type' => EventRibbon::TYPE_TASK_START,
            'messageEvent' => 'Вы выбраны исполнителем',
            'titleMail' => "Ваш отклик к заданию " . Html::encode($task->title) . " был принят",
            'messageMail' => "<p>Ваш отклик к заданию " . Html::a(Html::encode($task->title), "http://yii-taskforce/tasks/view/$task->id") . " был принят.</p>",
        ]);
    }
     ...
     * @param ActiveRecord $task - объект задачи
     * */
    public static function taskComplete(ActiveRecord $executorTask, ActiveRecord $task): void
    {
        self::send($executorTask, $task, [
            'type' => EventRibbon::TYPE_TASK_COMPLETE,
            'messageEvent' => 'Задание завершено',
            'titleMail' => "Задание " . Html::encode($task->title) . " завершено",
            'messageMail' => "<p>Заказчик завершил взятое вами задание " . Html::a(Html::encode($task->title), "http://yii-taskforce/tasks/view/$task->id") . ".</p>",
        ]);
    }
     ...
     * @param ActiveRecord $task - объект задачи
     * */
    public static function taskDenial(ActiveRecord $executorTask, ActiveRecord $task): void
    {
        self::send($executorTask, $task, [
            'type' => EventRibbon::TYPE_TASK_DENIAL,
            'messageEvent' => 'Исполнитель отказался от задания',
            'titleMail' => "Исполнитель отказался от задания " . Html::encode($task->title),
            'messageMail' => "<p>Исполнитель отказался от задания " . Html::a(Html::encode($task->title), "http://yii-taskforce/tasks/view/$task->id") . ".</p>",
        ]);
    }
     ...
     * @param ActiveRecord $task - объект задачи
     * */
    public static function taskMessage(ActiveRecord $user, ActiveRecord $task): void
    {
        self::send($user, $task, [
            'type' => EventRibbon::TYPE_NEW_CHAT_MESSAGE,
            'messageEvent' => 'Новое сообщение',
            'titleMail' => "К заданию " . Html::encode($task->title) . " было оставлено новое сообщение",
            'messageMail' => "<p>К заданию " . Html::a(Html::encode($task->title), "http://yii-taskforce/tasks/view/$task->id") . " было оставлено новое сообщение.</p>",
        ]);
    }
     ...
     * */
    private static function send(ActiveRecord $user, ActiveRecord $task, array $data): void
    {
        (new EventRibbon([
            'user_id' => $user->id,
            'task_id' => $task->id,
